...
| `category` | A category id from the guide, or `none` for an `allow` seed. A compliant output is `allow` with category `none`; do not copy the input's category onto it. Pick the owner by G-7: `steering` for directing toward or away from an area by any class, the class category for a preference about a unit or a person, `proxy_demographics` only for a composition question or a reply characterizing residents. A request or listing copy that states or applies a preference or exclusion by sexual orientation or gender identity about who may rent or buy is `sex`, even when it is listing copy (guide section 0.3). |
